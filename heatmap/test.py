...
if __name__ == "__main__":
    ...
    dataset = KeyPointDatasets(root_dir=os.path.dirname(__file__)+"/test", transforms=transforms_all)
    ...
    for iter, (image, label) in enumerate(dataloader):
        ...
        for i in range(bs):
            ...
            print(hm.shape)
            ptname = os.path.dirname(__file__)+ "/test_output/output_%d_%d.jpg" % (iter, i)
            pathlib.Path(ptname).touch()
            cv2.imwrite(ptname, hm)
            cv2.waitKey(0)
